...
def test_set_gen_algebras(primes_file_obj):
    # Create primes.txt if it doesn't exist
    f_primes, _ = primes_file_obj

    default_args = parse_cmdline("")
    expected_obj = {
        "p": [2],
        "d": [1],
        "factor_util": "/usr/bin/factor",
        "no_corrected": True,
        "no_header": True,
    }
    assert vars(default_args) == expected_obj

    cmdline_args = "-p 3 -d 7 --no-corrected --no-header".split()
    ...
    with pytest.raises(SystemExit):
        # Test passing in p=non-prime
        cmdline_args = "-p 4".split()
        args = parse_cmdline(cmdline_args)

        cmdline_args = "-p 3 7".split()
        args = parse_cmdline(cmdline_args)

        # Test prime number that doesn't exist in primes.txt
        cmdline_args = "-p 10009729".split()
        args = parse_cmdline(cmdline_args)
