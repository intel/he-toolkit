...
@pytest.fixture
def primes_file_obj():
    # primes.txt is generated by running gen_primes.sh
    # check tools first to see if file exists
    f_primes = r"primes.txt"
    gen_primes = r"tools/gen_primes.sh"

    pfile_exists = os.path.exists(f_primes)
    subprocess.call(gen_primes, shell=True)
    primesObj = PrimesFromFile(f_primes)

    # delete only if file didn't exist beforehand
    if not pfile_exists:
        os.remove(f_primes)
...
def test_PrimesFromFile_obj(primes_file_obj):
    primes_obj = primes_file_obj
    assert primes_obj.max == 139999
    assert len(primes_obj.primes) == 13010
