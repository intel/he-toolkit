...
def doTrain(Xtrain, ytrain, Xtest, ytest, epochs=10, verbose=False):
  ...
  for i in range(epochs):
    learning_rate = 10. / ((i + 1) + 1)
    new_lmbda = (1. + np.sqrt(1 + 4 * lmbda**2)) / 2.
...
def generateSynData(nSamples, nFeatures):
  ...
  data = sklearn.datasets.make_classification(n_samples=nSamples,
  ...
  Xtrain, Xtest, ytrain, ytest = train_test_split(X_scaled,
                                                  ...
                                                  shuffle=True,
                                                  stratify=y)
  Xtest, Xeval, ytest, yeval = train_test_split(Xtest,
                                                ytest,
                                                test_size=0.5,
                                                random_state=50,
                                                shuffle=True,
...
if __name__ == "__main__":
  ...
  if args.samples == 0 and args.features == 0 and args.name == None:
    print(
        "=== Synthetic data generation for logistic regression HE example ===")
    l_dataname = [
        "lrtest_small", "lrtest_mid", "lrtest_large", "lrtest_xlarge"
    ]
    l_features = [40, 80, 120, 200]
    l_samples = [500, 2000, 10000, 50000]

    for dataname, n_features, n_samples in zip(l_dataname, l_features,
                                               l_samples):
  ...
  # if all arguments are set, make custom data
    ...
    print(
        "=== Synthetic data generation for logistic regression HE example ===")
    print(" - Generating custom dataset :", args.name, " n_samples:",
          args.samples, " n_features:", args.features)
    X_train, y_train, X_test, y_test, X_eval, y_eval = generateSynData(
